export default function SummaryCard({ data }) {
  const { summary, reviews_sent_to_ai, processing_stats } = data;
  const sampleCount = reviews_sent_to_ai || processing_stats?.sampled || null;

  return (
    <div
      id="summary-card"
      className="bg-steam-card rounded-2xl border border-steam-border p-5 sm:p-6 shadow-xl"
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-2">
          <svg className="w-4 h-4 text-steam-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456z" />
          </svg>
          SteamLens Review Summary
        </h3>

        {sampleCount && (
          <span className="text-xs text-gray-400 bg-white/5 border border-white/10 px-2.5 py-1 rounded-full">
            Synthesized from {sampleCount} curated reviews
          </span>
        )}
      </div>

      <p className="text-gray-200 text-sm sm:text-base leading-relaxed">
        {summary}
      </p>
    </div>
  );
}


